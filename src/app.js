...
async function resolvePlayer(username){const urls=[`https://api.minecraftservices.com/minecraft/profile/lookup/name/${encodeURIComponent(username)}`,`https://api.mojang.com/users/profiles/minecraft/${encodeURIComponent(username)}`];for(const url of urls){for(let attempt=0;attempt<2;attempt++){try{const response=await fetch(url);if(response.ok)return response.json();if(response.status===404)break}catch{}await new Promise(resolve=>setTimeout(resolve,200))}}throw Object.assign(new Error('Minecraft player not found.'),{status:404})}
async function getCollectionResources(){if(collectionResources.data&&Date.now()-collectionResources.time<60*60*1000)return collectionResources.data;const response=await fetch('https://api.hypixel.net/v2/resources/skyblock/collections');if(!response.ok)throw new Error('Collection resources are temporarily unavailable.');const data=await response.json();collectionResources={time:Date.now(),data:data.collections||{}};return collectionResources.data}
...
async function getProfile(username, requestedId, force=false) {
  ...
  if (force || !raw || Date.now() - raw.time > CACHE_MS) {
    ...
    const hypixel = await fetch(`https://api.hypixel.net/v2/skyblock/profiles?uuid=${player.id}`, { headers: { 'API-Key': hypixelApiKey } });

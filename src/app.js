...
function bestiaryStats(member){const kills=member.bestiary?.kills||{},thresholds=[1,5,15,30,50,100,250,500,1000,2500,5000,10000],tierFor=value=>{let tier=0;for(const required of thresholds)if(value>=required)tier++;return tier},mobs=Object.entries(kills).filter(([id,value])=>id!=='last_killed_mob'&&typeof value==='number').map(([id,value])=>{const amount=Number(value),tier=tierFor(amount),next=thresholds[tier]||null;return{id,name:titleCase(id.replace(/_\d+$/,'')),kills:amount,tier,maxTier:thresholds.length,nextKills:next?Math.max(0,next-amount):0,tierProgress:next?Math.min(100,Math.round((amount-(thresholds[tier-1]||0))/(next-(thresholds[tier-1]||0))*100)):100}}).sort((a,b)=>b.kills-a.kills),completedTiers=mobs.reduce((sum,x)=>sum+x.tier,0),reportedLevel=[member.bestiary?.level,member.bestiary?.total_level,member.bestiary?.bestiary_level].find(value=>value!==null&&value!==undefined&&Number.isFinite(Number(value)));return{unique:mobs.length,totalKills:mobs.reduce((sum,x)=>sum+x.kills,0),completedTiers,level:reportedLevel===undefined?null:Number(reportedLevel),lastKilled:titleCase(kills.last_killed_mob||'Unknown'),mobs}}
...
function createServer(){return http.createServer(async (req, res) => {
  try {
    ...
    const appRoutes=new Set(['/inventory','/ender-chest','/backpacks','/bags','/sacks','/wardrobe','/equipment','/loadouts','/slayer','/dungeons','/pets','/mining','/garden','/bestiary','/networth','/notebook','/accessories','/mayor','/events','/ironpath']);
    const playerRoute=/^\/[A-Za-z0-9_]{1,16}(?:\/(?:inventory|ender-chest|backpacks|bags|sacks|wardrobe|equipment|loadouts|slayer|dungeons|pets|mining|garden|bestiary|networth|notebook|accessories|mayor|events|ironpath))?\/?$/;

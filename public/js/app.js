...
let currentStorage={inventory:[],enderChest:[],personalVault:[],backpacks:[],bags:[],sacks:[]};
let currentProfile=null;
const tabPaths={armorSets:'/wardrobe',equipmentSets:'/equipment',loadouts:'/loadouts',inventory:'/inventory',enderChest:'/ender-chest',backpacks:'/backpacks',bags:'/bags',sacks:'/sacks'};
...
let ironpathGoals=[],ironpathKey='',ironpathTimer;
...
function durationText(seconds){seconds=Math.max(0,Math.ceil(Number(seconds)||0));if(!seconds)return 'Ready';const days=Math.floor(seconds/86400),hours=Math.floor(seconds%86400/3600),minutes=Math.ceil(seconds%3600/60);return[days&&`${days}d`,hours&&`${hours}h`,minutes&&`${minutes}m`].filter(Boolean).join(' ')}
function ironpathPlan(recipeId,quantity){const data=currentProfile.ironpath||{},recipes=data.recipes||[],byId=Object.fromEntries(recipes.map(x=>[x.id,x])),pool={...(data.counts||{})},missing={},total={},time={value:0};const raw=(id,amount)=>{const recipe=byId[id];if(!recipe){total[id]=(total[id]||0)+amount;return}for(const item of recipe.ingredients)raw(item.id,item.count*amount)};const use=(id,amount)=>{const owned=Math.min(Number(pool[id]||0),amount);pool[id]=Number(pool[id]||0)-owned;const left=amount-owned;if(!left)return;const recipe=byId[id];if(!recipe){missing[id]=(missing[id]||0)+left;return}time.value+=recipe.duration*left;for(const item of recipe.ingredients)use(item.id,item.count*left)};raw(recipeId,quantity);use(recipeId,quantity);const materials=Object.entries(total).map(([id,needed])=>({id,name:recipes.flatMap(x=>x.ingredients).find(x=>x.id===id)?.name||id,needed,missing:Number(missing[id]||0)})).sort((a,b)=>b.missing-a.missing);const all=materials.reduce((s,x)=>s+x.needed,0),have=materials.reduce((s,x)=>s+(x.needed-x.missing),0);return{materials,time:time.value,percent:all?Math.round(have/all*100):100}}
async function saveIronpath(){if(!currentPlayer||!currentProfile)return;await fetch(`/api/ironpath/${encodeURIComponent(currentPlayer)}?profile=${encodeURIComponent(currentProfile.id)}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({goals:ironpathGoals})})}
...
$('#moduleContent').addEventListener('click',e=>{const remove=e.target.closest('[data-ironpath-remove]');if(remove){ironpathGoals=ironpathGoals.filter(x=>x.id!==Number(remove.dataset.ironpathRemove));drawIronpath();saveIronpath();return}if(e.target.closest('#newNote')){const note={id:Date.now(),title:'Untitled note',body:'',updated:Date.now()};notebookNotes.unshift(note);activeNoteId=note.id;renderNotebookEditor();saveNotebook();return}const link=e.target.closest('[data-note-id]');if(link){activeNoteId=Number(link.dataset.noteId);renderNotebookEditor();return}if(e.target.closest('#deleteNote')){notebookNotes=notebookNotes.filter(x=>x.id!==activeNoteId);activeNoteId=notebookNotes[0]?.id||null;renderNotebookEditor();saveNotebook()}});
$('#moduleContent').addEventListener('submit',e=>{if(e.target.id!=='ironpathAdd')return;e.preventDefault();const recipeId=$('#ironpathRecipe').value,quantity=Math.max(1,Math.min(64,Number($('#ironpathQuantity').value)||1));ironpathGoals.push({id:Date.now(),recipeId,quantity});drawIronpath();saveIronpath()});

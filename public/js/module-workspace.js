...
closeModule=function(updateRoute=true){workspaceCloseBase(updateRoute);document.querySelectorAll('#moduleSidebar [data-module],#moduleNav [data-module],#moduleSecondary [data-module],#moduleEssentials [data-module]').forEach(button=>{button.classList.remove('active');button.removeAttribute('aria-current')})};
...
for(const selector of ['#moduleEssentials','#moduleSecondary'])$(selector).addEventListener('click',event=>{const button=event.target.closest('[data-module]');if(!button)return;openModule(button.dataset.module);if(currentProfile&&button.dataset.accessoryView){accessoryTab=button.dataset.accessoryView;drawAccessories()}});
